# World-Navigation: definition workflow
...
3. Use its buttons to exercise move with c#, move with component, pause, resume, stop.
4. Inspect the configured hosts and triggers, then open
   [WorldNavigationWorkflow.cs](../Samples~/DefinitionWorkflow/Runtime/WorldNavigationWorkflow.cs). It is the caller
...
- **Move with C#**: `WorldNavigationWorkflow.Move()`.
- **Move with component**: `WorldNavigationWorkflow.MoveComponent()`.
- **Pause**: `WorldNavigationWorkflow.Pause()`.
- **Resume**: `WorldNavigationWorkflow.Resume()`.
- **Stop**: `WorldNavigationWorkflow.Stop()`.
